...
<!--
TODO: Not `Deref`, but get pointer value (*) ?

The reason is that `Box<Asteroid>` is _not_ the same thing as `Asteroid`. It is
its own type. But then, how were we able to call methods on it in the first
place? It turns out that Rust's core library exports a trait called
[`Deref`](https://doc.rust-lang.org/std/ops/trait.Deref.html), which is defined
like this:
...
As you might have guessed, `Box` implements this trait. You can think of it as
...
impl<T> Deref for Box<T> {
    type Target = T;
    fn deref(&self) -> &T {
        // Return a reference to the value stored behind the pointer.
        &*self
    }
}
```

In other words, whenever we wrote this:
...
Rust will do a bit more than _just_ call a method on `asteroid`. At compile-time,
it will go through the methods of `Box<Asteroid>` and see whether there exists
one called `render()`. Seeing that there is none, Rust will see whether `Deref`
is implemented on `asteroid`. It will go through every `impl` block and check
for one in which `Target` has a `render(&self, &mut Phi)` method.

It just so happen that the compiler has monomorphized the `impl Deref` block, a
fancy way of saying that the compiler has automatically written, for us,

```rust
impl Deref for Box<Asteroid> {
    type Target = Asteroid;
    fn deref(&self) -> &Asteroid {
        // Return a reference to the value stored behind the pointer.
        &*self
    }
}
...
Unlike `drop()`, you can manually call `deref()` in your code... but there isn't
much point to it:
...
let x = Box::new(3i32);
println!("{}", x.deref().pow(4));

// You can do the same thing
println!("{}", x.deref().pow(4));
```

This is all good... except when it isn't. In this case, the way in which `Deref`
has been defined prevents us from moving data around. See, there is no way for
the compiler to ensure that we return the same value every time. We could easily
break its trust and do something like this:
