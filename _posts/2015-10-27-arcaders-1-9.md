...
Your code should now compile and produce a black screen. Let's fill it up!
...
components used by multiple views but which do not belong to `Phi`. Do not
forget to import `Background` in `views/game.rs`, to mark its attributes public,
and to declare the `shared` module in `views/mod.rs`.
...
The answer will be in the snapshot.
Good luck!
