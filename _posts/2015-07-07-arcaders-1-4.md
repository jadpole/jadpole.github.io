---
layout: post
title:  "[DEPRECATED] ArcadeRS 1.4: Views"
categories: arcaders
---
...
we start working with external resources. One important thing to notice, though,
is that both of the attributes of phi depend on the same lifetime `'a'`. This is because they both depend on _not outliving_ the same _thing_. And that thing is
the `sdl_context`.

As always, lifetimes, when properly understood, are a powerful way to convey
invariants about our data structures &mdash; they are as important of a way to
talk to other programmers as they are of a way to talk to the compiler.

Before we go any further, now seems to be a good moment to organize a bit our
files.
...
Before I let you go, though, I would like to talk about the principle of
_composition over inheritance_. This is an aspect of Rust's philosophy that
often confuse newbies familiar with classical object-oriented languages such as
Ruby or C++.

Say you want to declare two structures, `Magician` and `Sorcerer`, both
describing a `Character` and having a `cast_magic` method, which only those two
have. In C++, you would typically create an abstract class `Character` which
defines the basic methods shared by all characters, and a `SpellCaster` which
declares an abstract (empty) `cast_magic` method and extends `Character`. Then,
you would let both `Magician` and `Sorcerer` extend `Character` and implement
`cast_magic`. This is called _inheritance_.

The main advantage of inheritance is that a child class has access to the
attributes of its parent class,  This way, you can tightly package your
abstractions, while allowing super-efficient code-reuse. Rust... doesn't have
that. Some would say that what it has is _better_; I would argue that what it
has is _simpler_.

In Rust, you would instead declare `Character` and `SpellCaster` as traits:
...
trait Character {
    ...
    // The default implementation depends on `set_health` to work
    fn receive_damage(&mut self, dmg: i32) -> i32 {
        let real_damage = ::std::cmp::min(self.get_health(), dmg);
        self.set_health(self.get_health() - real_damage);
        real_damage
    }

    fn attack(&self, enemy: &mut Enemy) -> i32 {
        enemy.receive_damage(self.get_strength())
    }
}

trait SpellCaster {
    fn cast_magic(&self, enemy: &mut Character) -> i32;
}
...
impl Character for Magician {
    h
}
```

One of the problems with this approach is that, now, anyone can call `set_health`
with any value, even a negative one. You could require that one only implements
`receive_damage` instead &mdash; which is what I would do in this situation &mdash;
however it is a _lot_ of code that you have to repeat.

So, why would we go for this obviously-inferior solution? Well, it turns out
that you can actually go quite far without
-->
