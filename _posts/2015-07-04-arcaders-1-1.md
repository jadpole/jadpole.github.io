...
The type system _forces_ us to deal with the possibility of getting an error
when we want to get the returned value. In this case, we simply `unwrap()` the
value, which means that we print the error message and panic, _safely crashing_
the program, if the result is an `Err(String)`, and that we assign the context
to `sdl_context` if the result is of type `Ok(Sdl)`.
...
tradition in the Rust world, it will _look weird_, but the `renderer` provides
an interface from which we cannot crash the program.
...
That was all for the first part of this series. [Next time](/arcaders/2015/07/06/arcaders-1-2),
