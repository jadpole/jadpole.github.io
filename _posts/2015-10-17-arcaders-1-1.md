...
If you wonder why the `=` is necessary, this is because Cargo, by default,
assumes that, by `"a.b.c"`, you really mean `"a.*.*"`. This is because,
according to SemVer, minor version bumps should not contain breaking changes,
and Cargo prefers that your code be up-to-date with the latest bug fixes.
However, because Rust-SDL2 is still far from stable (pre-1.0.0), we must select
a specific version in order to prevent our code from breaking randomly.
...
... you should see a window appear on your screen for 3 seconds, then close by
itself. And that was it! Now for the fun part: understanding what we just wrote.
...
We would then replace every occurrence of `sdl2` by `pineapple` and the result
...
What this means concretely is that:
...
The first approach is just cleaner in most (but not all) situations.
...
The nice thing about binding a feature to an identifier is that, once it goes
out of scope (at the end of the main function), all of the resources owned by
this part of the SDL library will be automatically freed. In fact, even if we
_panic_ at some point in the program, the destructor will still be called as
usual, preventing memory leaks. In other words, even crashing the program does
not break Rust's memory safety. I'll explain the concept of _panics_ soon enough,
but let's first focus on error handling.

Requesting a functionality returns a `Result<Handler, String>`, where `Handler`
is the type of the component we wish to use. Unlike most languages, Rust uses
_types_, instead of exceptions, to ensure that errors are propagated. This is
what you will typically find in a typed functional language such as
...
You might guess what comes next: when it sees this function, the compiler
...
_safely crashing_ the program. If the unwrapped value is of type `Ok(Sdl)`, then
the wrapped value is simply returned and assigned to `sdl_context`.

You should note, however, that unwrapping is frowned upon in most situations and
should only be used when there is no point to continue the program's execution.
In this game, we will use it whenever we wish to access resources that are
bundled with our game, or when we assert that something should be impossible.

You may notice that we did not use `unwrap` in our code, but `expect`. This
method plays the same role, but allows us to print our own error message instead
of the default one provided by SDL. That is,
...
It just results in a nicer experience for our end-user (provided that said user
...
OpenGL-based (thus, faster) rendering.

The resulting window is unwrapped such that, if it could not be created, the
program will panic with the error message "Could not open the main window".
...
On the first line, we set the _brush color_ to black (red=0, green=0, blue=0).
...
![A window not presented](/images/arcade-2.png)
...
features that seem like minor additions, but make Rust a pleasure to work with.
