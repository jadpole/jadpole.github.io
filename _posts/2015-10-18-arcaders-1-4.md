...
As always, skim through it. Get the principle, we'll work out the details.
...
outlive it: it would be a use-after-free bug. Therefore, `Phi` also depends on
the window being alive, which we make explicit through lifetimes annotations.
...
_Static dispatch_ allows functions to receive arguments of many types without
having to rewrite the code &mdash; as long as these arguments implement whatever
traits are needed to get the job done. For example, one could write a function
which sums up a `Vec` of _things_ (`T`).
...
This is similar what you have to write in many languages which have generic
...
work from us. This means, for C++ developers, that there is no need to write
