...
Notice that, while `render`'s signature ends with a semicolon, `resume` and
`pause` are followed by empty blocks. This means that these two have a
_default implementation_, here "do not do anything". In their signature, we also
add an underscore before `context`, so as to tell the compiler: I know that I'm
not using this value, it's done on purpose.
...
<!--[TODO]
Talk about the previous version, where the lifetimes were shared?
Then the lifetime's name becomes `video`?
-->
