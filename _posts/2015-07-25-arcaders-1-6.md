...
match event {
    Window { win_event_id: Resized, .. } => {
        self.now.resize = (renderer.get_output_size().unwrap());
    },
...
In this file, we will store our fundamental data structures, including
`Rectangle`, of course, but also extending to other niceties such as resource
pools. However, this is a story for another time...
...
__[The complete code until now](/code/arcaders-1-6/)__
