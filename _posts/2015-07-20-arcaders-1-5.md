...
One thing that we _will_ fix, though, is the necessity to implement `pause` and
`resume`. Most of the time, we will not need these, as resource management will
be handled while constructing the views. And as having them in the way doesn't
serve much purpose, we can provide a default implementation by modifying the
`View` trait:
...
This is how you provide a _default implementation_ in Rust!

Also, akin to `_` meaning _I do not care about this value_, `_context` means
_I may not care about this value_. It is to be noted that the value is indeed
bound to the name `_context`, but the compiler will not complain if we do not
use it.
